...
echo -e "\n------------\nStarting movie2map $(date)\n------------\n" >> $output_dir/log/err.log
...
if [[ $run_simulate == "yes" ]] ; then
    get_start
    ./auto_sim_ref.sh
    check_exit_status "Simulation" $output_dir
    get_stop
...
total_lines=$(wc -l <movie_full.txt)
lines_per_file=$(( ($total_lines + 4 - 1) / 4 ))
split -l $lines_per_file movie_full.txt
