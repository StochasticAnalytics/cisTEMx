...
idx1=$(awk 'BEGIN{found=0;idx=0}{if($1 != "C" && found==0 && $5 < 0.143) {found=1; idx=FNR-1}}END{print idx}' /tmp/local_stats.txt)
# Interpolate to find 0.143
resolution=$(awk -v I=$idx1 'BEGIN{x=0;y=0;x1=0;y1=0;x2=0;y2=0}{if($1 != "C" && FNR==I) {x1=$5;y1=$2}; if($1 != "C" && FNR==I+1) {x2=$5;y2=$2}}END{y=y1+(0.143-x1)*(y2-y1)/(x2-x1); print y}' /tmp/local_stats.txt)
{
echo "local recon stop time $stop_time" 
echo "local recon start time $start_time" 
echo -e "\nFound a resolution of $resolution after local search\n"
...
echo -e "Result: $result, local resolution $resolution, expected $expected_resolution\n"
echo -e "---------------------------------------------------\n"
