...
void CTF::SetEnvelope(float wanted_acceleration_voltage, float wanted_pixel_size_angstrom, float dose_rate) {
    ...
    squared_illumination_aperture = 16000.0f * dose_rate / (pi_v<float> * beam_brightness);
...
float CTF::PhaseShiftGivenSquaredSpatialFrequencyAndDefocus(float squared_spatial_frequency, float defocus) {
    return pi_v<float> * wavelength * squared_spatial_frequency * (defocus - 0.5f * squared_wavelength * squared_spatial_frequency * spherical_aberration) + additional_phase_shift + precomputed_amplitude_contrast_term;
}

float CTF::PhaseShiftGivenSquaredSpatialFrequencyAndAzimuth(float squared_spatial_frequency, float azimuth) {
    return pi_v<float> * wavelength * squared_spatial_frequency * (DefocusGivenAzimuth(azimuth) - 0.5f * squared_wavelength * squared_spatial_frequency * spherical_aberration) + additional_phase_shift + precomputed_amplitude_contrast_term;
}
...
float CTF::PhaseShiftGivenBeamTiltAndShift(float squared_spatial_frequency, float beam_tilt, float particle_shift) {
    float spatial_frequency = sqrtf(squared_spatial_frequency);
    float phase_shift       = 2.0f * pi_v<float> * spherical_aberration * squared_wavelength * squared_spatial_frequency * spatial_frequency * beam_tilt;
    phase_shift -= 2.0f * pi_v<float> * spatial_frequency * particle_shift;
...
void CTF::EnforceConvention( ) {
    ...
    if ( defocus_1 < defocus_2 ) {
        ...
        astigmatism_azimuth += pi_v<float> * 0.5f;
    }
    astigmatism_azimuth -= pi_v<float> * roundf(astigmatism_azimuth / pi_v<float>);
}
