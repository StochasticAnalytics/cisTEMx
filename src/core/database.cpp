...
void Database::GetCompletedImageAssetIdsForSearch(int search_id, std::set<long>& completed_ids) {
    // Clear the set before populating
    completed_ids.clear( );

    if ( search_id <= 0 ) {
        return; // No search to query
    }

    // Query all IMAGE_ASSET_IDs that have results for this search
    wxString query = wxString::Format(
            "SELECT DISTINCT IMAGE_ASSET_ID FROM TEMPLATE_MATCH_LIST WHERE SEARCH_ID = %d",
            search_id);

    bool more_data = BeginBatchSelect(query.ToUTF8( ).data( ));

    while ( more_data ) {
        long image_asset_id;
        more_data = GetFromBatchSelect("l", &image_asset_id);
        if ( more_data ) {
            completed_ids.insert(image_asset_id);
        }
    }
...
void Database::AddTemplateMatchingResult(long wanted_template_match_id, TemplateMatchJobResults& job_details) {
    ...
    InsertOrReplace("TEMPLATE_MATCH_LIST", "pllliiliitrrrrrrrrrr",
                    ...
                    job_details.search_type_code, job_details.parent_search_id, job_details.image_asset_id,
                    job_details.reference_volume_asset_id, 1, // IS_ACTIVE = 1 for new results
...
TemplateMatchJobResults Database::GetTemplateMatchingResultByID(long wanted_template_match_id) {
    ...
    sql_select_command = wxString::Format(
            ...
            "WHERE list.TEMPLATE_MATCH_ID = %li",
            wanted_template_match_id);
    ...
    temp_result.search_type_code          = sqlite3_column_int(list_statement, 5);
    temp_result.parent_search_id          = sqlite3_column_int64(list_statement, 6); // PARENT_SEARCH_ID
    temp_result.image_asset_id            = sqlite3_column_int64(list_statement, 7);
    temp_result.reference_volume_asset_id = sqlite3_column_int64(list_statement, 8);
...
long Database::AddToTemplateMatchQueue(const wxString& job_name, int image_group_id, int reference_volume_asset_id, int run_profile_id,
    ...
    sqlite3_bind_int(stmt, 6, run_profile_id);
...
#ifdef cisTEM_debug_duplicate_TM_list
    // Development-only check for duplicate IMAGE_ASSET_ID entries in TEMPLATE_MATCH_LIST
    // This should never happen - each IMAGE_ASSET_ID should appear at most once per SEARCH_ID
    if ( search_id > 0 ) {
        wxString dup_check_sql = wxString::Format(
                "SELECT COUNT(*) - COUNT(DISTINCT IMAGE_ASSET_ID) as dup_count "
                "FROM TEMPLATE_MATCH_LIST WHERE SEARCH_ID = %ld",
                search_id);
        int dup_count = ReturnSingleIntFromSelectCommand(dup_check_sql);

        if ( dup_count > 0 ) {
            // Found duplicate entries - automatically clean them up during development
            wxPrintf("WARNING: Found %d duplicate IMAGE_ASSET_ID entries for SEARCH_ID %ld in TEMPLATE_MATCH_LIST.\n", dup_count, search_id);
            wxPrintf("         This indicates a bug in the resume logic. Automatically deleting all results for this search.\n");

            // Delete all entries for this SEARCH_ID
            wxString delete_sql = wxString::Format("DELETE FROM TEMPLATE_MATCH_LIST WHERE SEARCH_ID = %ld", search_id);
            ExecuteSQL(delete_sql);

            // Reset the SEARCH_ID in the queue table so it can be reassigned
            wxString reset_sql = wxString::Format("UPDATE TEMPLATE_MATCH_QUEUE SET SEARCH_ID = -1 WHERE SEARCH_ID = %ld", search_id);
            ExecuteSQL(reset_sql);

            wxPrintf("         Cleanup complete - deleted all results for SEARCH_ID %ld. The search can now be rerun.\n", search_id);

            // Return 0/N to reflect that results were cleared
            if ( image_group_id == -1 ) {
                wxString count_sql = "SELECT COUNT(*) FROM IMAGE_ASSETS";
                int      total     = ReturnSingleIntFromSelectCommand(count_sql);
                return std::make_pair(0, total);
            }
            else {
                wxString count_sql = wxString::Format("SELECT COUNT(*) FROM IMAGE_GROUP_%d", image_group_id);
                int      total     = ReturnSingleIntFromSelectCommand(count_sql);
                return std::make_pair(0, total);
            }
        }
