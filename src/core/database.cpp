...
RefinementPackage* Database::GetNextRefinementPackage( ) {
    ...
    while ( return_code == SQLITE_ROW ) {
        ...
        temp_info.particle_group                      = sqlite3_column_int(list_statement, 14);
        temp_info.total_exposure                      = sqlite3_column_double(list_statement, 15);
...
Refinement* Database::GetRefinementByID(long wanted_refinement_id, bool include_particle_info) {
    ...
    if ( include_particle_info == true ) {
        for ( class_counter = 0; class_counter < temp_refinement->number_of_classes; class_counter++ ) {
            ...
            while ( more_data == true ) {
                more_data = GetFromBatchSelect("lsssssssssssssissssssssiis",
                                               &temp_result.position_in_stack,
                                               &temp_result.psi,
                                               &temp_result.theta,
                                               &temp_result.phi,
                                               &temp_result.xshift,
        ...
        if (std::get<COLUMN_CHANGE_NAME>(column) == "DROP_TABLE") {
            ...
            if ( progress_bar ) {
                ...
                if ( current_progress > previous_progress ) {
                    progress_bar->OnUpdateProgress(current_progress, wxString::Format("Dropping obsolete table %s...", table_to_drop), should_update_text);
