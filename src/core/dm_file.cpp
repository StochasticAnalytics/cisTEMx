...
*/
    ...
    if ( sb ) {
        if ( fixed ) {
            if ( show )
                MyDebugPrint("Warning: Swapping header byte order for 4-byte types\n");
            for ( i = 0; i < 24; i += 4 )
                swapbytes(b + i, 4);
        }
        else {
            ...
            if ( show )
                MyDebugPrint("Warning: Swapping header byte order for 2-byte types\n");
            for ( i = 0; i < 8; i += 2 )
                swapbytes(b + i, 2);
        }
    }
    ...
    if ( readdata ) {
        ...
        if ( fimg->fail( ) )
            return -3;
        if ( sb )
            swapbytes(alloc_size( ), p, data_type_size( ));
    }
...
int DMFile::readTagGroupWithVersion(std::ifstream* fimg, unsigned char* p, bool readdata, int img_select) {
    ...
    if ( show == 1 ) {
        if ( sb )
            std::cout << "Warning: Swapping header bytes" << std::endl;
...
int DMFile::readTag(std::ifstream* fimg, int dim_flag, unsigned char* p, bool readdata, int& notag) {
    ...
    *fimg >> t;
    ...
    if ( t == 20 ) { // TagSubGroup record
        if ( strcmp((char*)tag, "Dimensions") == 0 )
            dim_flag = 1;
        ...
        if ( dt <= 12 ) {
            ...
            if ( dtarr[1] == 15 ) { // Struct
                ...
                if ( strcmp((char*)tag, "Data") == 0 && nel > 1e6 ) {
                    ...
                    for ( i = 0; i < nel; i++ ) {
                        if ( i < 10 )
                            val = dm3_value(fimg, dtarr[1]); // Element value
                        else
                            fimg->read((char*)buf, dtlen); // Element value
                    }
                }
            }
            level--;
        ...
        if ( keep && strcmp((char*)tag, "ImageData") == 0 )
            keep = 0;
        ...
        if ( keep ) {
            if ( strcmp((char*)tag, "DataType") == 0 ) {
                ...
                if ( show == 1 )
                    std::cout << "\tdatatype=" << data_type( ) << "\tcompoundtype=" << compound_type( ) << "\tc=" << channels( ) << "\tval=" << val;
            }
            ...
            if ( dim_flag == 1 ) {
                ...
                dim_flag = 0;
                if ( show == 1 )
                    std::cout << "\tx=" << sizeX( ) << "\ty=" << sizeY( ) << "\tn=" << images( );
            }
            ...
            if ( strcmp((char*)tag, "Pixel Size (um)") == 0 ) {
                ...
                if ( show == 1 )
                    std::cout << "\t" << tag << "=" << samplingX( ) << samplingY( ) << samplingZ( ) << std::endl;
            ...
            if ( strstr((char*)tag, "Emission") ) {
                if ( show == 1 ) {
                    std::cout << "\t" << tag << "=" << val << std::endl;
                    for ( i = 0; i < len; i++ )
                        std::cout << (int)tag[i] << "\t" << tag[i] << std::endl;
                }
            }
        }
    }
...
double DMFile::dm3_value(std::ifstream* fimg, int dm3_type) {
    ...
    if ( sb - endianness )
        swapbytes((unsigned char*)buf, dtlen);
...
unsigned long DMFile::dm_read_integer(std::ifstream* fimg, long len) {
    ...
    if ( sb )
        swapbytes((unsigned char*)buf, len);
...
size_t DMFile::data_type_size( ) {
    ...
    switch ( datatype ) {
        case Bit:
        case UCharacter:
        case SCharacter: typesize = sizeof(char); break;
        case UShort:
        case Short: typesize = sizeof(short); break;
        case UInteger:
        case Integer: typesize = sizeof(int); break;
        case ULong:
        case Long: typesize = sizeof(long); break;
