...
void Reconstruct3D::InsertSliceWithCTF(Particle& particle_to_insert, float symmetry_weight) {
    ...
    particle_weight = particle_to_insert.particle_occupancy / particle_to_insert.parameter_average.get<cp_t::occupancy>( ) / powf(particle_to_insert.sigma_noise / particle_to_insert.parameter_average.get<cp_t::sigma>( ), 2);
#else
        ...
        for ( j = int_y_coordinate; j <= int_y_coordinate + 1; j++ ) {
            ...
            if ( i >= 0 ) {
                ...
                for ( k = int_z_coordinate; k <= int_z_coordinate + 1; k++ ) {
                    if ( i <= image_reconstruction.logical_upper_bound_complex_x && j >= image_reconstruction.logical_lower_bound_complex_y && j <= image_reconstruction.logical_upper_bound_complex_y && k >= image_reconstruction.logical_lower_bound_complex_z && k <= image_reconstruction.logical_upper_bound_complex_z ) {
                        if ( k >= 0 ) {
                ...
                physical_coord_xy = upper_x * physical_y_address + physical_x_address;
                for ( k = int_z_coordinate; k <= int_z_coordinate + 1; k++ ) {
                    if ( i >= image_reconstruction.logical_lower_bound_complex_x && j >= image_reconstruction.logical_lower_bound_complex_y && j <= image_reconstruction.logical_upper_bound_complex_y && k >= image_reconstruction.logical_lower_bound_complex_z && k <= image_reconstruction.logical_upper_bound_complex_z ) {
                        if ( k > 0 ) {
                            physical_z_address = image_reconstruction.logical_z_dimension - k;
                        }
                        else {
...
float Reconstruct3D::Correct3DCTF(Image& buffer3d) {
    ...
    for ( i = 0; i <= buffer3d.real_memory_allocated / 2; i++ )
        buffer3d.complex_values[i] = ctf_reconstruction[i];
    ...
    for ( i = 0; i <= buffer3d.real_memory_allocated / 2; i++ )
        ctf_reconstruction[i] = real(buffer3d.complex_values[i]);
...
void Reconstruct3D::ReadArrayHeader(wxString filename, int& logical_x_dimension, int& logical_y_dimension, int& logical_z_dimension,
    ...
    symmetry_symbol = "";
    for ( i = 0; i < 4; i++ )
        symmetry_symbol += temp_char[count + i];
    count += 4;
    char_pointer = (char*)&oddeven;
    for ( i = 0; i < sizeof(int); i++ ) {
        char_pointer[i] = temp_char[count];
...
void Reconstruct3D::ReadArrays(wxString filename) {
    ...
    for ( i = 0; i < 4; i++ )
        input_symmetry_symbol += temp_char[count + i];
    count += 4;
    char_pointer = (char*)&oddeven;
    for ( i = 0; i < sizeof(int); i++ ) {
        char_pointer[i] = temp_char[count];
