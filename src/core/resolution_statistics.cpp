...
float ResolutionStatistics::ReturnEstimatedResolution(bool use_part_fsc) {
    ...
    if ( estimated_resolution < 2.0f * pixel_size )
        estimated_resolution = 2.0f * pixel_size;
...
void ResolutionStatistics::NormalizeVolumeWithParticleSSNR(Image& reconstructed_volume) {
    ...
    if ( need_fft )
        reconstructed_volume.ForwardFFT( );
    ...
    if ( need_fft )
        reconstructed_volume.BackwardFFT( );
...
void ResolutionStatistics::CalculateFSC(Image& reconstructed_volume_1, Image& reconstructed_volume_2, bool smooth_curve) {
    ...
    if ( number_of_bins <= 1 )
        number_of_bins = reconstructed_volume_1.ReturnSmallestLogicalDimension( ) / 2 + 1;
    int number_of_bins2 = 2 * (number_of_bins - 1);

    // Extend table to include corners in 3D Fourier space
    if ( reconstructed_volume_1.logical_z_dimension == 1 )
        number_of_bins_extended = int(number_of_bins * sqrtf(2.0)) + 1;
    else
        number_of_bins_extended = int(number_of_bins * sqrtf(3.0)) + 1;
    ...
    if ( smooth_curve ) {
        if ( window > number_of_bins_extended / 10 )
            window = number_of_bins_extended / 10;
        if ( window < 5 )
            window = 5;

        if ( IsOdd(window) == false )
            window++;
...
void ResolutionStatistics::RestrainParticleSSNR(float low_resolution_limit) {
    ...
    if ( low_resolution_limit > 0.0f )
        reciprocal_resolution_limit = 1.0f / low_resolution_limit;
    Curve temp_curve;
    ...
    for ( i = 0; i < part_SSNR.NumberOfPoints( ); i++ ) {
        if ( temp_curve.data_x[i] >= reciprocal_resolution_limit ) {
            ...
            if ( adjustment_factor < -FLT_MAX * 0.5f && temp_curve.gaussian_fit[i] > 0.0f )
                adjustment_factor = part_SSNR.data_y[i] / powf(temp_curve.gaussian_fit[i], 2);
            ...
            if ( adjustment_factor >= -FLT_MAX * 0.5f )
