...
void EulerSearch::Run<GpuImage>(Particle& particle, Image& input_3d, GpuImage* projections, int batch_size) {
    ...
    for ( batch.index = 0; batch.index < batch.n_batches( ); ++batch.index ) {
        for ( int intra_batch_idx = 0; intra_batch_idx < batch.n_images_in_this_batch( ); intra_batch_idx += batch.intra_loop_inc( ) ) {
            ...
            tmp_rot.ForwardFFT( );
            // The result as written gives a quadrant swapped correlation map. I want a map with the origin at the center of the image.
            // We could a) quadrand swap the correlation map (every iteration) or b) just not swap this origin here, which via associativity is like appliying the swap here & then the swap at the end.
            // TODO: make sure this does not affect the MirrorYFourier2d op. If so, this could be re-applied to the reference projections in refine3d, which would cost more (instead of saving ops here)
    ...
    for ( int iSearchPosition = 0; iSearchPosition < number_of_search_positions; iSearchPosition++ ) {
        ...
        for ( batch.index = 0; batch.index < batch.n_batches( ); batch.index++ ) {
            ...
            gpu_correlation_map.object_is_centred_in_box = true;
