...
ActionsPanelRx::ActionsPanelRx(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style)
    : ActionsPanelParent(parent, id, pos, size, style) {
    // Bind OnListBookPageChanged from
    Bind(wxEVT_LISTBOOK_PAGE_CHANGED, wxBookCtrlEventHandler(ActionsPanelRx::OnActionsBookPageChanged), this);
}

// TODO: destructor

void ActionsPanelRx::OnActionsBookPageChanged(wxBookCtrlEvent& event) {
