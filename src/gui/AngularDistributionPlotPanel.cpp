...
void AngularDistributionPlotPanel::DrawAxisUnderlay( ) {
    float tmp_x = 0.0;
    float tmp_y = 0.0;
...
void AngularDistributionPlotPanel::SetupBitmap( ) {
    if ( buffer_bitmap.GetHeight( ) > 5 && buffer_bitmap.GetWidth( ) > 5 ) {
        ...
        if ( draw_axis_overlay_instead_of_underlay == true )
            DrawAxisOverlay(0, min_number_of_projections_to_be_red);
    }
}
    ...
    for ( int sym_counter = 0; sym_counter < symmetry_matrices.number_of_matrices; sym_counter++ ) {
        ...
        for ( int y = 0; y < myroundint(proj_circle_radius * 2); ++y ) {
            ...
            for ( int x = 0; x < myroundint(proj_circle_radius * 2); ++x, ++p ) {
                if ( pow(start_x + x, 2) + pow(start_y + y, 2) > circle_radius_squared )
                    continue;
                ...
                else if ( current_red_value == current_blue_value && current_red_value == current_green_value ) // grey, must be label
                {
                    p.Red( )   = 0;
                    p.Green( ) = 0;
                    p.Blue( )  = 128;
                }
                else if ( current_red_value == 0 && current_green_value == 0 && current_blue_value < 255 ) {
                    current_blue_value += colour_change_step;
                ...
                else if ( current_red_value == 0 && current_blue_value == 255 && current_green_value < 255 ) {
                ...
                else if ( current_blue_value == 255 && current_green_value == 255 && current_red_value < 128 ) {
                    current_red_value += colour_change_step;
                ...
                else if ( current_green_value == 255 && current_blue_value > 128 && current_red_value < 255 ) {
                    current_red_value += colour_change_step;
                    current_blue_value -= colour_change_step;
                ...
                else if ( current_green_value == 255 && current_red_value == 255 && current_blue_value > 0 ) {
                    current_blue_value -= colour_change_step;
                    if ( current_blue_value < 0 ) {
                        current_green_value += current_blue_value;
                        current_blue_value = 0;
                ...
                else if ( current_red_value == 255 && current_blue_value == 0 && current_green_value > 0 ) {
                    current_green_value -= colour_change_step;

                    if ( current_green_value < 0 )
...
void AngularDistributionPlotPanel::UpdateScalingAndDimensions( ) {
    ...
    UpdateProjCircleRadius( );
    if ( panel_dim_x > 0 && panel_dim_y > 0 )
        SetupBitmap( );
}
...
void AngularDistributionPlotPanel::UpdateProjCircleRadius( ) {
    ...
    if ( proj_circle_radius < 0.5f )
        proj_circle_radius = 0.5f; // we can't do less than 1 pixel
    ...
    if ( draw_axis_overlay_instead_of_underlay == true )
        colour_change_step = myroundint(1021.0f / (red_value));
    else
        colour_change_step = myroundint(1022.0f / (red_value));

    if ( colour_change_step < 1 )
        colour_change_step = 1; // can't be less than 1.
...
void AngularDistributionPlotPanel::DrawAxisOverlay(int min_value, int max_value) {
    wxDouble label_width;
    wxDouble label_height;
    wxDouble label_descent;
    wxDouble label_externalLeading;
...
void AngularDistributionPlotPanelHistogram::DrawPlot(int min_value, int max_value) {
    ...
    p.MoveTo(data, myroundint(circle_center_x - circle_radius), myroundint(circle_center_y - circle_radius));
    ...
    for ( y_position = -myroundint(circle_radius); y_position <= myroundint(circle_radius); y_position++ ) {
        ...
        for ( x_position = -myroundint(circle_radius); x_position <= myroundint(circle_radius); x_position++ ) {
            ...
            if ( current_radius_squared <= circle_radius_squared ) {
                ...
                current_colour = GetColourBarValue(current_value, min_value, max_value);
