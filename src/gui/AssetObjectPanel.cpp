...
AssetObjectPanel::AssetObjectPanel(wxWindow* parent)
...
AssetObjectPanel::~AssetObjectPanel( ) {
    delete all_groups_list;
}

void AssetObjectPanel::RemoveAssetFromGroups(long wanted_asset, bool dirty_groups) {
    long counter;
    long found_position;
    ...
    if ( dirty_groups == true )
        DirtyGroups( );
}

void AssetObjectPanel::OnDisplayButtonClick(wxCommandEvent& event) {
    DisplaySelectedItems( );
}

void AssetObjectPanel::DisplaySelectedItems( ) {
...
void AssetObjectPanel::RemoveAssetClick(wxCommandEvent& event) {
            ...
            if ( ShowDeleteMessageDialog( ) == wxID_YES ) {
                ...
                for ( ;; ) {
                    ...
                    for ( removed_counter = 0; removed_counter < number_removed; removed_counter++ ) {
                        if ( already_removed[removed_counter] < item )
                            adjusted_item--;
                    }
...
void AssetObjectPanel::AddContentItemToGroup(long wanted_group, long wanted_content_item) {
...
void AssetObjectPanel::AddArrayItemToGroup(long wanted_group, long wanted_array_item) {
    MyDebugAssertTrue(wanted_group > 0 && wanted_group < all_groups_list->number_of_groups, "Requesting a group (%li) that doesn't exist!", wanted_group)
...
void AssetObjectPanel::DeleteArrayItemFromGroup(long wanted_group, long wanted_array_item) {
    MyDebugAssertTrue(wanted_group > 0 && wanted_group < all_groups_list->number_of_groups, "Requesting a group (%li) that doesn't exist!", wanted_group)
            MyDebugAssertTrue(wanted_array_item >= 0 && wanted_array_item < all_assets_list->number_of_assets, "Requesting an asset(%li) that doesn't exist!", wanted_array_item)
...
void AssetObjectPanel::AddArrayofArrayItemsToGroup(long wanted_group, wxArrayLong* array_of_wanted_items, OneSecondProgressDialog* progress_dialog) {
    MyDebugAssertTrue(wanted_group > 0 && wanted_group < all_groups_list->number_of_groups, "Requesting a group (%li) that doesn't exist!", wanted_group)
    ...
    for ( long counter = 0; counter < array_of_wanted_items->GetCount( ); counter++ ) {
        MyDebugAssertTrue(array_of_wanted_items->Item(counter) >= 0 && array_of_wanted_items->Item(counter) < all_assets_list->number_of_assets, "Requesting an asset(%li) that doesn't exist!", array_of_wanted_items->Item(counter)) if ( all_groups_list->groups[wanted_group].FindMember(array_of_wanted_items->Item(counter)) == -1 ) {
...
void AssetObjectPanel::AddSelectedAssetClick(wxCommandEvent& event) {
    // How many assets are selected?
...
void AssetObjectPanel::RemoveAllAssetsClick(wxCommandEvent& event) {
    long counter;
...
void AssetObjectPanel::NewGroupClick(wxCommandEvent& event) {
...
void AssetObjectPanel::RemoveGroupClick(wxCommandEvent& event) {
    if ( selected_group != 0 ) {
        // database..
...
void AssetObjectPanel::RenameGroupClick(wxCommandEvent& event) {
...
void AssetObjectPanel::InvertGroupClick(wxCommandEvent& event) {
    //
...
void AssetObjectPanel::NewFromParentClick(wxCommandEvent& event) {
    MyDebugAssertTrue(false, "Oops, this method should never be called. You need to override it in the panel class");
}

void AssetObjectPanel::OnGroupActivated(wxListEvent& event) {
    GroupListBox->EditLabel(event.GetIndex( ));
}

void AssetObjectPanel::OnAssetActivated(wxListEvent& event) {
    DisplaySelectedItems( );
}

void AssetObjectPanel::AddAsset(Asset* asset_to_add) {
    ...
    if ( asset_to_add->asset_id >= current_asset_number )
        current_asset_number = asset_to_add->asset_id + 1;

    //FillContentsList();
}

void AssetObjectPanel::SetSelectedGroup(long wanted_group) {
...
unsigned long AssetObjectPanel::ReturnNumberOfAssets( ) {
    return all_assets_list->number_of_assets;
}

unsigned long AssetObjectPanel::ReturnNumberOfGroups( ) {
    return all_groups_list->number_of_groups;
}

void AssetObjectPanel::SetGroupName(long wanted_group, wxString wanted_name) {
...
wxString AssetObjectPanel::ReturnGroupName(long wanted_group) {
    return all_groups_list->groups[wanted_group].name;
}

int AssetObjectPanel::ReturnGroupID(long wanted_group) {
    return all_groups_list->groups[wanted_group].id;
}

wxString AssetObjectPanel::ReturnAssetShortFilename(long wanted_asset) {
    return all_assets_list->ReturnAssetPointer(wanted_asset)->ReturnShortNameString( );
}

wxString AssetObjectPanel::ReturnAssetLongFilename(long wanted_asset) {
    return all_assets_list->ReturnAssetPointer(wanted_asset)->ReturnFullPathString( );
}

long AssetObjectPanel::ReturnGroupMember(long wanted_group, long wanted_member) {
    return all_groups_list->groups[wanted_group].members[wanted_member];
}

int AssetObjectPanel::ReturnGroupMemberID(long wanted_group, long wanted_member) {
    return all_assets_list->ReturnAssetPointer(all_groups_list->groups[wanted_group].members[wanted_member])->asset_id;
}

long AssetObjectPanel::ReturnGroupSize(long wanted_group) {
    return all_groups_list->groups[wanted_group].number_of_members;
}

void AssetObjectPanel::SizeGroupColumn( ) {
    int client_height;
    int client_width;
    ...
    if ( client_width > current_width )
        GroupListBox->SetColumnWidth(0, client_width);
}

void AssetObjectPanel::FillGroupList( ) {
...
void AssetObjectPanel::OnBeginEdit(wxListEvent& event) {
...
void AssetObjectPanel::OnEndEdit(wxListEvent& event) {
...
void AssetObjectPanel::RenameAssetClick(wxCommandEvent& event) {
...
void AssetObjectPanel::SizeContentsColumn(int column_number) {
...
void AssetObjectPanel::FillContentsList( ) {
    if ( selected_group >= 0 ) {
        ...
        if ( all_groups_list->groups[selected_group].number_of_members > 0 )
            ContentsListBox->RefreshItems(0, all_groups_list->groups[selected_group].number_of_members - 1);
...
void AssetObjectPanel::OnGroupFocusChange(wxListEvent& event) {
...
void AssetObjectPanel::OnContentsSelected(wxListEvent& event) {
...
void AssetObjectPanel::OnBeginContentsDrag(wxListEvent& event) {
...
void AssetObjectPanel::MouseVeto(wxMouseEvent& event) {
    //Do nothing
}

void AssetObjectPanel::MouseCheckContentsVeto(wxMouseEvent& event) {
    VetoInvalidMouse(ContentsListBox, event);
}

void AssetObjectPanel::MouseCheckGroupsVeto(wxMouseEvent& event) {
    VetoInvalidMouse(GroupListBox, event);
}

void AssetObjectPanel::VetoInvalidMouse(wxListCtrl* wanted_list, wxMouseEvent& event) {
...
void AssetObjectPanel::OnMotion(wxMouseEvent& event) {
    if ( should_veto_motion == false )
        event.Skip( );
}

/*
bool AssetObjectPanel::IsFileAnAsset(wxFileName file_to_check)
{
...
int AssetObjectPanel::ReturnArrayPositionFromParentID(int wanted_id) {
    return all_assets_list->ReturnArrayPositionFromParentID(wanted_id);
}

int AssetObjectPanel::ReturnArrayPositionFromAssetID(int wanted_id) {
    return all_assets_list->ReturnArrayPositionFromID(wanted_id);
}

long AssetObjectPanel::ReturnParentAssetID(long wanted_asset) {
    return all_assets_list->ReturnParentAssetID(wanted_asset);
}

int AssetObjectPanel::ReturnAssetID(int wanted_asset) {
    return all_assets_list->ReturnAssetID(wanted_asset);
}

wxString AssetObjectPanel::ReturnAssetName(long wanted_asset) {
    return all_assets_list->ReturnAssetName(wanted_asset);
}

bool AssetObjectPanel::DragOverGroups(wxCoord x, wxCoord y) {
...
void AssetObjectPanel::EnableNewFromParentButton( ) {
    //NewFromParentButton->Enable(false);
}

void AssetObjectPanel::OnUpdateUI(wxUpdateUIEvent& event) {
    if ( main_frame->current_project.is_open == false ) {
        ...
        if ( ContentsListBox->GetItemCount( ) < 1 ) {
            ...
            if ( ContentsListBox->GetSelectedItemCount( ) < 1 ) {
                ...
                if ( ReturnNumberOfGroups( ) > 1 )
                    AddSelectedAssetButton->Enable(true);
                else
                    AddSelectedAssetButton->Enable(false);
            }
        }
...
GroupDropTarget::GroupDropTarget(wxListCtrl* owner, AssetObjectPanel* asset_panel) {
    my_owner = owner;
...
wxDragResult GroupDropTarget::OnDragOver(wxCoord x, wxCoord y, wxDragResult defResult) {
    if ( my_panel->DragOverGroups(x, y) == true )
        return wxDragCopy;
    else
        return wxDragCancel;
}
