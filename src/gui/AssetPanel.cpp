...
void AssetPanel::RemoveAssetFromGroups(long wanted_asset, bool dirty_groups) {
    long counter;
    long found_position;
...
void AssetPanel::OnDisplayButtonClick(wxCommandEvent& event) {
    DisplaySelectedItems( );
}

void AssetPanel::DisplaySelectedItems( ) {
...
void AssetPanel::RemoveAssetClick(wxCommandEvent& event) {
...
void AssetPanel::AddContentItemToGroup(long wanted_group, long wanted_content_item) {
...
void AssetPanel::AddArrayItemToGroup(long wanted_group, long wanted_array_item) {
    MyDebugAssertTrue(wanted_group > 0 && wanted_group < all_groups_list->number_of_groups, "Requesting a group (%li) that doesn't exist!", wanted_group)
...
void AssetPanel::DeleteArrayItemFromGroup(long wanted_group, long wanted_array_item) {
    MyDebugAssertTrue(wanted_group > 0 && wanted_group < all_groups_list->number_of_groups, "Requesting a group (%li) that doesn't exist!", wanted_group)
            MyDebugAssertTrue(wanted_array_item >= 0 && wanted_array_item < all_assets_list->number_of_assets, "Requesting an asset(%li) that doesn't exist!", wanted_array_item)
...
void AssetPanel::AddArrayofArrayItemsToGroup(long wanted_group, wxArrayLong* array_of_wanted_items, OneSecondProgressDialog* progress_dialog) {
    MyDebugAssertTrue(wanted_group > 0 && wanted_group < all_groups_list->number_of_groups, "Requesting a group (%li) that doesn't exist!", wanted_group)
...
void AssetPanel::AddSelectedAssetClick(wxCommandEvent& event) {
    // How many assets are selected?
...
void AssetPanel::RemoveAllAssetsClick(wxCommandEvent& event) {
    long counter;
...
void AssetPanel::NewGroupClick(wxCommandEvent& event) {
...
void AssetPanel::RemoveGroupClick(wxCommandEvent& event) {
    if ( selected_group != 0 ) {
        // database..
...
void AssetPanel::RenameGroupClick(wxCommandEvent& event) {
...
void AssetPanel::InvertGroupClick(wxCommandEvent& event) {
    //
...
void AssetPanel::NewFromParentClick(wxCommandEvent& event) {
    MyDebugAssertTrue(false, "Oops, this method should never be called. You need to override it in the panel class");
}

void AssetPanel::OnGroupActivated(wxListEvent& event) {
    GroupListBox->EditLabel(event.GetIndex( ));
}

void AssetPanel::OnAssetActivated(wxListEvent& event) {
    DisplaySelectedItems( );
}

void AssetPanel::AddAsset(Asset* asset_to_add) {
...
void AssetPanel::SetSelectedGroup(long wanted_group) {
...
unsigned long AssetPanel::ReturnNumberOfAssets( ) {
    return all_assets_list->number_of_assets;
}

unsigned long AssetPanel::ReturnNumberOfGroups( ) {
    return all_groups_list->number_of_groups;
}

void AssetPanel::SetGroupName(long wanted_group, wxString wanted_name) {
...
void AssetPanel::OnBeginEdit(wxListEvent& event) {
...
void AssetPanel::OnEndEdit(wxListEvent& event) {
...
void AssetPanel::RenameAssetClick(wxCommandEvent& event) {
...
void AssetPanel::SizeContentsColumn(int column_number) {
...
void AssetPanel::FillContentsList( ) {
    if ( selected_group >= 0 ) {
...
void AssetPanel::OnGroupFocusChange(wxListEvent& event) {
...
void AssetPanel::OnContentsSelected(wxListEvent& event) {
...
void AssetPanel::OnBeginContentsDrag(wxListEvent& event) {
...
void AssetPanel::MouseVeto(wxMouseEvent& event) {
    //Do nothing
}

void AssetPanel::MouseCheckContentsVeto(wxMouseEvent& event) {
    VetoInvalidMouse(ContentsListBox, event);
}

void AssetPanel::MouseCheckGroupsVeto(wxMouseEvent& event) {
    VetoInvalidMouse(GroupListBox, event);
}

void AssetPanel::VetoInvalidMouse(wxListCtrl* wanted_list, wxMouseEvent& event) {
...
void AssetPanel::OnMotion(wxMouseEvent& event) {
    if ( should_veto_motion == false )
        event.Skip( );
}

/*
bool AssetPanel::IsFileAnAsset(wxFileName file_to_check)
{
...
int AssetPanel::ReturnArrayPositionFromParentID(int wanted_id) {
    return all_assets_list->ReturnArrayPositionFromParentID(wanted_id);
}

int AssetPanel::ReturnArrayPositionFromAssetID(int wanted_id) {
    return all_assets_list->ReturnArrayPositionFromID(wanted_id);
}

long AssetPanel::ReturnParentAssetID(long wanted_asset) {
    return all_assets_list->ReturnParentAssetID(wanted_asset);
}

int AssetPanel::ReturnAssetID(int wanted_asset) {
    return all_assets_list->ReturnAssetID(wanted_asset);
}

wxString AssetPanel::ReturnAssetName(long wanted_asset) {
    return all_assets_list->ReturnAssetName(wanted_asset);
}

bool AssetPanel::DragOverGroups(wxCoord x, wxCoord y) {
...
void AssetPanel::EnableNewFromParentButton( ) {
    //NewFromParentButton->Enable(false);
}

void AssetPanel::OnUpdateUI(wxUpdateUIEvent& event) {
...
GroupDropTarget::GroupDropTarget(wxListCtrl* owner, AssetPanel* asset_panel) {
    my_owner = owner;
