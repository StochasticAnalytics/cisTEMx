...
AssetsPanel::AssetsPanel(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style)
    : AssetsPanelParent(parent, id, pos, size, style) {
    // Bind OnListBookPageChanged from
    Bind(wxEVT_LISTBOOK_PAGE_CHANGED, wxBookCtrlEventHandler(AssetsPanel::OnAssetsBookPageChanged), this);
}

// TODO: destructor

void AssetsPanel::OnAssetsBookPageChanged(wxBookCtrlEvent& event) {
