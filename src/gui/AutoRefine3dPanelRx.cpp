...
AutoRefine3DPanelRx::AutoRefine3DPanelRx(wxWindow* parent)
    ...
    RefinementPackageSelectPanel->AssetComboBox->Bind(wxEVT_COMMAND_COMBOBOX_SELECTED, &AutoRefine3DPanelRx::OnRefinementPackageComboBox, this);
    Bind(wxEVT_AUTOMASKERTHREAD_COMPLETED, &AutoRefine3DPanelRx::OnMaskerThreadComplete, this);
    Bind(wxEVT_MULTIPLY3DMASKTHREAD_COMPLETED, &AutoRefine3DPanelRx::OnMaskerThreadComplete, this);
    Bind(RETURN_PROCESSED_IMAGE_EVT, &AutoRefine3DPanelRx::OnOrthThreadComplete, this);
...
void AutoRefine3DPanelRx::Reset( ) {
...
void AutoRefine3DPanelRx::SetInfo( ) {
...
void AutoRefine3DPanelRx::OnInfoURL(wxTextUrlEvent& event) {
...
void AutoRefine3DPanelRx::ResetAllDefaultsClick(wxCommandEvent& event) {
    // TODO : should probably check that the user hasn't changed the defaults yet in the future
    SetDefaults( );
}

void AutoRefine3DPanelRx::SetDefaults( ) {
...
void AutoRefine3DPanelRx::OnUpdateUI(wxUpdateUIEvent& event) {
...
void AutoRefine3DPanelRx::OnAutoMaskButton(wxCommandEvent& event) {
    auto_mask_value = AutoMaskYesRadioButton->GetValue( );
}

void AutoRefine3DPanelRx::OnUseMaskCheckBox(wxCommandEvent& event) {
    if ( UseMaskCheckBox->GetValue( ) == true ) {
...
void AutoRefine3DPanelRx::OnExpertOptionsToggle(wxCommandEvent& event) {
...
void AutoRefine3DPanelRx::FillRefinementPackagesComboBox( ) {
    if ( RefinementPackageSelectPanel->FillComboBox( ) == false )
        NewRefinementPackageSelected( );
}

void AutoRefine3DPanelRx::NewRefinementPackageSelected( ) {
...
void AutoRefine3DPanelRx::OnRefinementPackageComboBox(wxCommandEvent& event) {

    NewRefinementPackageSelected( );
}

void AutoRefine3DPanelRx::OnInputParametersComboBox(wxCommandEvent& event) {
    //SetDefaults();
}

void AutoRefine3DPanelRx::TerminateButtonClick(wxCommandEvent& event) {
    main_frame->job_controller.KillJob(my_job_id);
...
void AutoRefine3DPanelRx::FinishButtonClick(wxCommandEvent& event) {
    ProgressBar->SetValue(0);
    TimeRemainingText->SetLabel("Time Remaining : ???h:??m:??s");
    CancelAlignmentButton->Show(true);
    FinishButton->Show(false);
...
void AutoRefine3DPanelRx::StartRefinementClick(wxCommandEvent& event) {
    stopwatch.Start( );
    my_refinement_manager.BeginRefinementCycle( );
}

void AutoRefine3DPanelRx::WriteInfoText(wxString text_to_write) {
...
void AutoRefine3DPanelRx::WriteBlueText(wxString text_to_write) {
...
void AutoRefine3DPanelRx::WriteErrorText(wxString text_to_write) {
...
void AutoRefine3DPanelRx::FillRunProfileComboBoxes( ) {
    ReconstructionRunProfileComboBox->FillWithRunProfiles( );
    RefinementRunProfileComboBox->FillWithRunProfiles( );
}

void AutoRefine3DPanelRx::OnSocketJobResultMsg(JobResult& received_result) {
    my_refinement_manager.ProcessJobResult(&received_result);
}

void AutoRefine3DPanelRx::OnSocketJobResultQueueMsg(ArrayofJobResults& received_queue) {
...
void AutoRefine3DPanelRx::SetNumberConnectedText(wxString wanted_text) {
    NumberConnectedText->SetLabel(wanted_text);
}

void AutoRefine3DPanelRx::SetTimeRemainingText(wxString wanted_text) {
    TimeRemainingText->SetLabel(wanted_text);
}

void AutoRefine3DPanelRx::OnSocketAllJobsFinished( ) {
    my_refinement_manager.ProcessAllJobsFinished( );
}

void AutoRefine3DPanelRx::OnMaskerThreadComplete(wxThreadEvent& my_event) {
    if ( my_event.GetInt( ) == active_mask_thread_id )
        my_refinement_manager.OnMaskerThreadComplete( );
}

void AutoRefine3DPanelRx::OnOrthThreadComplete(ReturnProcessedImageEvent& my_event) {
