...
void DistributionPlotDialog::UpdateCurveToPlot(bool reset_bounds) {
    ...
    if ( plot_histogram_of_this_series[data_series_index] ) {
        ...
        if ( reset_bounds )
            HistogramComputeAutoBounds(data_series_y[data_series_index], number_of_points_in_data_series[data_series_index], lower_bound_x, upper_bound_x);
...
Curve HistogramFromArray(double* data, int number_of_data, int number_of_bins, double lower_bound, double upper_bound) {
    ...
    if ( lower_bound == 0.0 && upper_bound == 0.0 )
        HistogramComputeAutoBounds(data, number_of_data, lower_bound, upper_bound);
