...
void MyRefine2DPanel::OnStarFileWriteThreadComplete(wxThreadEvent& event) {
    ...
    else if ( my_classification_manager.running_job_type == REFINEMENT ) {
    ...
    if ( RefinementPackageComboBox->GetCount( ) > 0 ) {
        ...
        if ( InputParametersComboBox->GetSelection( ) > 0 ) {
            ...
            if ( InputParametersComboBox->GetSelection( ) == 0 ) {
                ...
                if ( calculated_number_of_classes > 50 )
                    calculated_number_of_classes = 50;
                else if ( calculated_number_of_classes > 40 )
                    calculated_number_of_classes = 40;
                else if ( calculated_number_of_classes > 30 )
                    calculated_number_of_classes = 30;
                else if ( calculated_number_of_classes > 20 )
                    calculated_number_of_classes = 20;
                else if ( calculated_number_of_classes > 10 )
                    calculated_number_of_classes = 10;
                else
...
void ClassificationManager::RunInitialStartJob( ) {
    ...
    if ( output_classification->percent_used > 100.0 )
...
void ClassificationManager::RunRefinementJobPostStarFileWrite(wxString input_star_file) {
    ...
    if ( output_classification->auto_percent_used == true ) {
        if ( number_of_rounds_to_run < 10 )
            output_classification->percent_used = 100.0;
        else if ( number_of_rounds_to_run < 20 ) {
            if ( number_of_rounds_run < 5 ) {
                output_classification->percent_used = (float(output_classification->number_of_classes * 300) / float(output_classification->number_of_particles)) * 100.0;
                if ( output_classification->percent_used > 100.0 )
                    output_classification->percent_used = 100.0;
            }
            else if ( number_of_rounds_run < number_of_rounds_to_run - 5 ) {
                output_classification->percent_used = (float(output_classification->number_of_classes * 300) / float(output_classification->number_of_particles)) * 100.0;
                if ( output_classification->percent_used > 100.0 )
                    output_classification->percent_used = 100.0;
                else if ( output_classification->percent_used < 30 )
                    output_classification->percent_used = 30.0;
            }
            else
                output_classification->percent_used = 100.0;
        }
        else if ( number_of_rounds_to_run < 30 ) {
    ...
    if ( output_classification->percent_used < min_percent_used )
        output_classification->percent_used = min_percent_used;
    ...
    for ( job_counter = 0; job_counter < number_of_refinement_jobs; job_counter++ ) {
        ...
        if ( job_counter == 0 )
            my_parent->WriteInfoText("Will automask reference class averages");
...
void ClassificationManager::ProcessJobResult(JobResult* result_to_process) {
    ...
    else if ( running_job_type == REFINEMENT ) {
        long current_image = long(result_to_process->result_data[0] + 0.5) - 1;
    ...
    if ( start_with_random == true ) {
        ...
        for ( long counter = 0; counter < input_classification->number_of_particles; counter++ ) {
            if ( output_classification->classification_results[counter].best_class > 0 ) {
                ...
                if ( long(abs(output_classification->classification_results[counter].best_class)) != long(abs(input_classification->classification_results[counter].best_class)) )
                    number_moved++;
            }
