...
void MyRelionExportDialog::OnExportButtonClick(wxCommandEvent& event) {
    ...
    if ( DownsamplingFactorSpinCtrl->GetValue( ) > 1 )
        box_large.Allocate(BoxSizeSpinCtrl->GetValue( ) * DownsamplingFactorSpinCtrl->GetValue( ), BoxSizeSpinCtrl->GetValue( ) * DownsamplingFactorSpinCtrl->GetValue( ), 1);
    ...
    for ( int image_counter = 0; image_counter < number_of_images_in_group; image_counter++ ) {
        ...
        for ( size_t particle_counter = 0; particle_counter < current_array_of_assets.GetCount( ); particle_counter++ ) {
            ...
            if ( NormalizeCheckBox->IsChecked( ) )
                box.ZeroFloatAndNormalize(1.0, particle_radius / (float(DownsamplingFactorSpinCtrl->GetValue( )) * current_image_asset->pixel_size), true); // this is for Relion, which likes background values to be normalized
