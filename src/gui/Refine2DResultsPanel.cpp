...
void Refine2DResultsPanel::FillInputParametersComboBox(void) {
    if ( RefinementPackageComboBox->GetSelection( ) >= 0 ) {
        ...
        else if ( InputParametersComboBox->GetSelection( ) == -1 ) {
            Clear( );
        }
    }
}
...
void Refine2DResultsPanel::WriteJobInfo(long wanted_classification_id) {
    ...
    if ( temp_classification->exclude_blank_edges == true )
        ExcludeBlankEdgesStaticText->SetLabel("Yes");
    else
        ExcludeBlankEdgesStaticText->SetLabel("No");

    if ( temp_classification->auto_percent_used == true )
        AutoPercentUsedStaticText->SetLabel("Yes");
    else
        AutoPercentUsedStaticText->SetLabel("No");
...
void Refine2DResultsPanel::OnUpdateUI(wxUpdateUIEvent& event) {
    if ( main_frame->current_project.is_open == false ) {
        ...
        if ( refinement_package_asset_panel->all_classification_selections.GetCount( ) - SelectionManagerListCtrl->all_valid_selections.GetCount( ) > 0 && InputParametersComboBox->GetSelection( ) >= 0 )
            CopyOtherButton->Enable(true);
        else
            CopyOtherButton->Enable(false);
    }
}
...
void Refine2DResultsPanel::OnDeselected(wxListEvent& event) {
    ...
    if ( SelectionManagerListCtrl->GetEditControl( ) != NULL )
        SelectionManagerListCtrl->EndEditLabel(false);
