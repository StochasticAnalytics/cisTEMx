...
RefineTemplateDevPanel::RefineTemplateDevPanel(wxWindow* parent)
    : RefineTemplateDevPanelParent(parent) {
    // Set variables
...
void RefineTemplateDevPanel::ResetAllDefaultsClick(wxCommandEvent& event) {
    ResetDefaults( );
}

void RefineTemplateDevPanel::OnInfoURL(wxTextUrlEvent& event) {
...
void RefineTemplateDevPanel::Reset( ) {
...
void RefineTemplateDevPanel::ResetDefaults( ) {
...
void RefineTemplateDevPanel::OnGroupComboBox(wxCommandEvent& event) {
    //	ResetDefaults();
    //	AssetGroup active_group;
    active_group.CopyFrom(&image_asset_panel->all_groups_list->groups[GroupComboBox->GetSelection( )]);
    if ( active_group.number_of_members > 0 ) {
...
void RefineTemplateDevPanel::SetInfo( ) {
...
void RefineTemplateDevPanel::FillRunProfileComboBox( ) {
    RunProfileComboBox->FillWithRunProfiles( );
}

void RefineTemplateDevPanel::OnUpdateUI(wxUpdateUIEvent& event) {
...
void RefineTemplateDevPanel::StartEstimationClick(wxCommandEvent& event) {
    ...
    for ( int image_counter = 0; image_counter < active_group.number_of_members; image_counter++ ) {
        ...
        temp_result.high_res_limit                  = 2.f; //FIXME
        ...
        current_job_package.AddJob("ttfffffffffffifffffbffttttttttttttttfffbtfiiiiiitft", input_search_image.ToUTF8( ).data( ),
                                   ...
                                   mask_radius_search,
                                   phase_shift,
                                   input_result.GetMipFilename( ).ToUTF8( ).data( ),
                                   input_result.GetScaledMipFilename( ).ToUTF8( ).data( ),
                                   input_result.GetPsiFilename( ).ToUTF8( ).data( ),
                                   input_result.GetThetaFilename( ).ToUTF8( ).data( ),
                                   input_result.GetPhiFilename( ).ToUTF8( ).data( ),
                                   input_result.GetDefocusFilename( ).ToUTF8( ).data( ),
                                   input_result.GetPixelSizeFilename( ).ToUTF8( ).data( ),
...
void RefineTemplateDevPanel::FinishButtonClick(wxCommandEvent& event) {
    ProgressBar->SetValue(0);
    TimeRemainingText->SetLabel("Time Remaining : ???h:??m:??s");
    CancelAlignmentButton->Show(true);
    FinishButton->Show(false);
...
void RefineTemplateDevPanel::TerminateButtonClick(wxCommandEvent& event) {
...
void RefineTemplateDevPanel::WriteInfoText(wxString text_to_write) {
...
void RefineTemplateDevPanel::WriteErrorText(wxString text_to_write) {
...
void RefineTemplateDevPanel::OnSocketJobResultMsg(JobResult& received_result) {
    if ( received_result.result_size > 0 ) {
        ProcessResult(&received_result);
    }
}

void RefineTemplateDevPanel::OnSocketJobResultQueueMsg(ArrayofJobResults& received_queue) {
    for ( int counter = 0; counter < received_queue.GetCount( ); counter++ ) {
        ProcessResult(&received_queue.Item(counter));
    }
}

void RefineTemplateDevPanel::SetNumberConnectedText(wxString wanted_text) {
    NumberConnectedText->SetLabel(wanted_text);
}

void RefineTemplateDevPanel::SetTimeRemainingText(wxString wanted_text) {
    TimeRemainingText->SetLabel(wanted_text);
}

void RefineTemplateDevPanel::OnSocketAllJobsFinished( ) {
    ProcessAllJobsFinished( );
}

void RefineTemplateDevPanel::ProcessResult(JobResult* result_to_process) // this will have to be overidden in the parent clas when i make it.
{
...
void RefineTemplateDevPanel::ProcessAllJobsFinished( ) {
...
void RefineTemplateDevPanel::WriteResultToDataBase( ) {
...
void RefineTemplateDevPanel::UpdateProgressBar( ) {
