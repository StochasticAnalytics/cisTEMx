...
void RefineTemplatePanel::StartEstimationClick(wxCommandEvent& event) {
    ...
    temp_result.db.search_type_code = cistem::job_type::template_match_refinement; //			float high_resolution_limit = resolution_limit;
    ...
    for ( int image_counter = 0; image_counter < active_group.number_of_members; image_counter++ ) {
        ...
        temp_result.db.parent_search_id = current_match_template_result_id;
        ...
        temp_result.db.datetime_of_run              = (long int)now.GetAsDOS( );
        ...
        temp_result.db.output_filename_base = main_frame->current_project.template_matching_asset_directory.GetFullPath( ) +
...
void RefineTemplatePanel::HandleSocketTemplateMatchResultReady(wxSocketBase* connected_socket, int& image_number, float& threshold_used, ArrayOfTemplateMatchFoundPeakInfos& peak_infos, ArrayOfTemplateMatchFoundPeakInfos& peak_changes) {
    ...
    current_input = main_frame->current_project.database.GetTemplateMatchingResultByID(cached_results[image_number - 1].db.parent_search_id);
...
void RefineTemplatePanel::WriteResultToDataBase( ) {
    ...
    for ( int counter = 0; counter < cached_results.GetCount( ); counter++ ) {
        cached_results[counter].db.search_id            = template_match_job_id;
        cached_results[counter].db.template_match_id    = template_match_id;
        cached_results[counter].db.elapsed_time_seconds = elapsed_seconds;
    ...
    for ( int counter = 0; counter < cached_results.GetCount( ); counter++ ) {
        main_frame->current_project.database.SetActiveTemplateMatchJobForGivenImageAssetID(cached_results[counter].db.image_asset_id, template_match_job_id);
    }
