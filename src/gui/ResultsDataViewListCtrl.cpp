...
ResultsDataViewListCtrl::ResultsDataViewListCtrl(wxWindow* parent, wxWindowID id, const wxPoint& pt, const wxSize& sz, long style) : wxDataViewListCtrl(parent, id, pt, sz, style) {
    my_parent                 = parent;
...
void ResultsDataViewListCtrl::SizeColumns( ) {
    ...
    if ( number_of_columns > 2 ) {
        ...
        if ( new_width > 100 )
            GetColumn(1)->SetWidth(new_width);
        else
...
void ResultsDataViewListCtrl::UncheckItem(const int row, const int column) {
    ...
    if ( value == CHECKED )
        SetValue(wxVariant(UNCHECKED), row, column);
    else if ( value == CHECKED_WITH_EYE )
        SetValue(wxVariant(UNCHECKED_WITH_EYE), row, column);
}
...
void ResultsDataViewListCtrl::CheckItem(const int row, const int column) {
    ...
    if ( value == UNCHECKED )
        SetValue(wxVariant(CHECKED), row, column);
    else if ( value == UNCHECKED_WITH_EYE )
        SetValue(wxVariant(CHECKED_WITH_EYE), row, column);
}
    ...
    if ( value == CHECKED || value == CHECKED_WITH_EYE ) // make sure there are no other checked items in this row..
    {
        for ( int column_counter = 2; column_counter < GetColumnCount( ); column_counter++ ) {
            if ( column_counter != column ) {
                ...
                if ( value == CHECKED || value == CHECKED_WITH_EYE )
                    UncheckItem(row, column_counter);
            }
        }
    }
...
void ResultsDataViewListCtrl::Deselect( ) {
    if ( currently_selected_column >= 0 && currently_selected_row >= 0 ) {
        ...
        if ( value == CHECKED_WITH_EYE )
            SetValue(wxVariant(CHECKED), currently_selected_row, currently_selected_column);
        else if ( value == UNCHECKED_WITH_EYE )
            SetValue(wxVariant(UNCHECKED), currently_selected_row, currently_selected_column);
...
CheckboxRenderer::CheckboxRenderer(const wxString& varianttype, wxDataViewCellMode mode, int align) : wxDataViewCustomRenderer(varianttype, mode, align) {
}
...
bool CheckboxRenderer::ActivateCell(const wxRect& cell, wxDataViewModel* model, const wxDataViewItem& item, unsigned int col, const wxMouseEvent* mouseEvent) {
    if ( mouseEvent != NULL ) {
        ...
        if ( mouse_x >= 0 && mouse_x <= 18 ) {
            ...
            //else
            if ( current_mode == UNCHECKED )
                model->ChangeValue(wxVariant(CHECKED), item, col);
            else
                //if (current_mode == CHECKED_WITH_EYE) model->ChangeValue(wxVariant(UNCHECKED_WITH_EYE), item, col);
                //else
                if ( current_mode == UNCHECKED_WITH_EYE )
                    model->ChangeValue(wxVariant(CHECKED_WITH_EYE), item, col);
        }
        else if ( mouse_x >= 20 && mouse_x <= 40 ) {
            if ( current_mode == CHECKED )
                model->ChangeValue(wxVariant(CHECKED_WITH_EYE), item, col);
            else if ( current_mode == UNCHECKED )
                model->ChangeValue(wxVariant(UNCHECKED_WITH_EYE), item, col);
        }
    }
...
bool CheckboxRenderer::Render(wxRect cell, wxDC* dc, int state) {
    if ( current_mode == UNCHECKED )
        dc->DrawBitmap(unchecked_bmp, cell.x, cell.y, true);
    else if ( current_mode == CHECKED )
        dc->DrawBitmap(checked_bmp, cell.x, cell.y, true);
    else if ( current_mode == UNCHECKED_WITH_EYE )
        dc->DrawBitmap(unchecked_eye_bmp, cell.x, cell.y, true);
    else if ( current_mode == CHECKED_WITH_EYE )
        dc->DrawBitmap(checked_eye_bmp, cell.x, cell.y, true);
