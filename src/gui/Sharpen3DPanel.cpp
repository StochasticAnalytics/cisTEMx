...
void Sharpen3DPanel::OnRunButtonClick(wxCommandEvent& event) {
    ...
    if ( active_refinement == NULL || UseFSCWeightingYesButton->GetValue( ) == false )
        input_resolution_statistics = NULL;
    else
        input_resolution_statistics = &active_refinement->class_refinement_results[active_class].class_resolution_statistics;
...
void Sharpen3DPanel::OnSharpenThreadComplete(ReturnSharpeningResultsEvent& my_event) {
    ...
    if ( active_thread_id == my_event.GetInt( ) ) {
        ...
        if ( sharpened_curve != NULL )
            delete sharpened_curve;

        if ( original_orth_image == NULL ) {
            if ( sharpened_orth_image != NULL )
                delete sharpened_orth_image;
        }
        else if ( sharpened_orth_image == NULL )
            delete original_orth_image;
        else // display them
        {
            Freeze( );
...
wxThread::ExitCode SharpenMapThread::Entry( ) {
    ...
    if ( DoesFileExist(map_filename) == true ) {
        ...
        if ( correct_sinc == true )
            volume_to_sharpen->CorrectSinc(outer_mask_radius);
