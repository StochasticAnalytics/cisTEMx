...
void ShowTemplateMatchResultsPanel::OnSavePeaksClick(wxCommandEvent& event) {
    if ( current_result.found_peaks.GetCount( ) > 0 ) {
        ...
        coordinate_file.WriteCommentLine(wxString::Format("Reference Volume File        : %s", volume_asset_panel->ReturnAssetLongFilename(volume_asset_panel->ReturnArrayPositionFromAssetID(current_result.reference_volume_asset_id))).ToUTF8( ).data( ));
...
void ShowTemplateMatchResultsPanel::OnPeakListSelectionChange(wxListEvent& event) {
    if ( PeakListCtrl->GetSelectedItemCount( ) == 0 )
        ...
        if ( current_result.search_type_code == cistem::job_type::template_match_refinement ) {
            int selected_change;
...
void ShowTemplateMatchResultsPanel::OnChangeListSelectionChange(wxListEvent& event) {
    if ( ChangesListCtrl->GetSelectedItemCount( ) > 0 && current_result.search_type_code == cistem::job_type::template_match_refinement ) {
        ...
        if ( current_result.peak_changes[selected_change].new_peak_number != -1 ) {
            int selected_peak;
            if ( PeakListCtrl->GetSelectedItemCount( ) == 0 )
                selected_peak = PeakListCtrl->GetNextItem(-1, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
            else
                selected_peak = -1;
...
void ShowTemplateMatchResultsPanel::SetActiveResult(TemplateMatchJobResults& result_to_show) {
    ...
    if ( current_result.search_type_code == cistem::job_type::template_match_full_search ) {
...
void ShowTemplateMatchResultsPanel::FillPeakInfoTable(float threshold_used) {
    ...
    if ( current_result.search_type_code == cistem::job_type::template_match_refinement ) {
        ...
        for ( counter = 0; counter < current_result.peak_changes.GetCount( ); counter++ ) {
            ...
            if ( current_result.peak_changes[counter].new_peak_number == -1 )
                ChangesListCtrl->InsertItem(counter, "NA");
            else
                ChangesListCtrl->InsertItem(counter, wxString::Format("%i", current_result.peak_changes[counter].new_peak_number));

            if ( current_result.peak_changes[counter].original_peak_number == -1 )
                ChangesListCtrl->SetItem(counter, 1, "NA");
            else
                ChangesListCtrl->SetItem(counter, 1, wxString::Format("%i", current_result.peak_changes[counter].original_peak_number));
...
void ShowTemplateMatchResultsPanel::DrawHistogram(wxString histogram_filename) {
    ...
    if ( DoesFileExist(histogram_filename) == true ) {
        ...
        for ( int counter = 0; counter < histogram_file.number_of_lines; counter++ ) {
            ...
            if ( values[0] > 0 ) {
                if ( values[2] > 0 )
                    log_survival_histogram = logf(values[2]);
                else
                    log_survival_histogram = 0.0f;

                if ( values[3] > 0 )
                    log_expected_survival_histogram = logf(values[3]);
                else
                    log_expected_survival_histogram = 0.0f;

                if ( log_expected_survival_histogram == 0.0f && log_survival_histogram == 0.0f )
                    break;
