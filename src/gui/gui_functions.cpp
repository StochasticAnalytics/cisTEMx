...
void GetMultilineTextExtent(wxDC* wanted_dc, const wxString& string, int& width, int& height) {
    ...
    for ( current_line = 0; current_line < number_of_lines; current_line++ ) {
        ...
        if ( line_size.GetWidth( ) > width )
            width = line_size.GetWidth( );
...
wxArrayString GetRecentProjectsFromSettings( ) {
    ...
    if ( wxConfig::Get( )->HasEntry("RecentProject1") == true )
        temp.Add(wxConfig::Get( )->Read("RecentProject1"));
    if ( wxConfig::Get( )->HasEntry("RecentProject2") == true )
        temp.Add(wxConfig::Get( )->Read("RecentProject2"));
    if ( wxConfig::Get( )->HasEntry("RecentProject3") == true )
        temp.Add(wxConfig::Get( )->Read("RecentProject3"));
    if ( wxConfig::Get( )->HasEntry("RecentProject4") == true )
        temp.Add(wxConfig::Get( )->Read("RecentProject4"));
    if ( wxConfig::Get( )->HasEntry("RecentProject5") == true )
        temp.Add(wxConfig::Get( )->Read("RecentProject5"));
...
void AddProjectToRecentProjects(wxString project_to_add) {
    ...
    for ( counter = recent_projects.GetCount( ) - 1; counter > 0; counter-- ) {
        if ( recent_projects.Item(counter) == project_to_add )
...
void FillGroupComboBoxWorker(wxComboBox* GroupComboBox, bool include_all_images_group) {
    ...
    if ( ! include_all_images_group )
        first_group_to_include = 1;
...
void FillParticlePositionsGroupComboBox(wxComboBox* GroupComboBox, bool include_all_particle_positions_group) {
    ...
    long first_group_to_include = 0;
    if ( ! include_all_particle_positions_group )
        first_group_to_include = 1;
