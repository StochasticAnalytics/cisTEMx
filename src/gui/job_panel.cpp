...
void JobPanel::HandleSocketNumberOfConnections(wxSocketBase* connected_socket, int received_number_of_connections) {
    ...
    if ( current_job_package.number_of_jobs < current_job_package.my_profile.ReturnTotalJobs( ) )
        ...
        total_processes = current_job_package.my_profile.ReturnTotalJobs( );

    int length_of_process_number;

    if ( total_processes >= 100000 )
        length_of_process_number = 6;
    else if ( total_processes >= 10000 )
        length_of_process_number = 5;
    else if ( total_processes >= 1000 )
        length_of_process_number = 4;
    else if ( total_processes >= 100 )
        length_of_process_number = 3;
    else if ( total_processes >= 10 )
        length_of_process_number = 2;
    else
        length_of_process_number = 1;

    if ( received_number_of_connections == total_processes )
        WriteInfoText(wxString::Format("All %i processes are connected.", received_number_of_connections));

    if ( length_of_process_number == 6 )
        SetNumberConnectedText(wxString::Format("%6i / %6i processes connected.", received_number_of_connections, total_processes));
    else if ( length_of_process_number == 5 )
        SetNumberConnectedText(wxString::Format("%5i / %5i processes connected.", received_number_of_connections, total_processes));
    else if ( length_of_process_number == 4 )
        SetNumberConnectedText(wxString::Format("%4i / %4i processes connected.", received_number_of_connections, total_processes));
    else if ( length_of_process_number == 3 )
        SetNumberConnectedText(wxString::Format("%3i / %3i processes connected.", received_number_of_connections, total_processes));
    else if ( length_of_process_number == 2 )
        SetNumberConnectedText(wxString::Format("%2i / %2i processes connected.", received_number_of_connections, total_processes));
    else
...
void JobPanel::SetNumberConnectedTextToZeroAndStartTracking( ) {
    ...
    if ( current_job_package.number_of_jobs < current_job_package.my_profile.ReturnTotalJobs( ) )
        number_of_refinement_processes = current_job_package.number_of_jobs;
    else
        number_of_refinement_processes = current_job_package.my_profile.ReturnTotalJobs( );

    int length_of_process_number;

    if ( number_of_refinement_processes >= 100000 )
        length_of_process_number = 6;
    else if ( number_of_refinement_processes >= 10000 )
        length_of_process_number = 5;
    else if ( number_of_refinement_processes >= 1000 )
        length_of_process_number = 4;
    else if ( number_of_refinement_processes >= 100 )
        length_of_process_number = 3;
    else if ( number_of_refinement_processes >= 10 )
        length_of_process_number = 2;
    else
        length_of_process_number = 1;

    if ( length_of_process_number == 6 )
        SetNumberConnectedText(wxString::Format("%6i / %6li processes connected.", 0, number_of_refinement_processes));
    else if ( length_of_process_number == 5 )
        SetNumberConnectedText(wxString::Format("%5i / %5li processes connected.", 0, number_of_refinement_processes));
    else if ( length_of_process_number == 4 )
        SetNumberConnectedText(wxString::Format("%4i / %4li processes connected.", 0, number_of_refinement_processes));
    else if ( length_of_process_number == 3 )
        SetNumberConnectedText(wxString::Format("%3i / %3li processes connected.", 0, number_of_refinement_processes));
    else if ( length_of_process_number == 2 )
        SetNumberConnectedText(wxString::Format("%2i / %2li processes connected.", 0, number_of_refinement_processes));
    else
