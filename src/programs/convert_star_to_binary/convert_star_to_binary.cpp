...
void ConvertStarToBin::DoInteractiveUserInput( ) {
    ...
    std::string input_filename  = my_input->GetFilenameFromUser("Input cisTEMx STAR file", "Filename of cisTEMx STAR file to convert", "input.star", true);
    std::string output_filename = my_input->GetFilenameFromUser("Output cisTEMx binary file", "converted output in the cisTEMx binary format", "output.cistem", false);
