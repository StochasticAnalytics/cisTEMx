...
bool MatchTemplateApp::DoCalculation( ) {
    ...
    input_image.ZeroCentralPixel( );
    // Note: we previously were dividing by the sqrt of the sum of squares, so the variance in the images was 1/N, not 1.
    // By setting the variance to 1 here, and not modifying the rest of the standardization/ normalization, the output CCF
    // becomes the normalized cross correlation (Var ~ `/N) * sqrt(N).
...
#pragma omp critical
            ...
            for ( current_search_position = first_search_position; current_search_position <= last_search_position; current_search_position++ ) {
                ...
                for ( current_psi = psi_start; current_psi <= psi_max; current_psi += psi_step ) {
                    ...
                    // This also makes the expected variance in the CCF proprotional to N * var(image), rather than n * var(template) * var(image). (where var(template) refers to the variance in the NON-padded template)
                    ...
                    // Note: The real space variance is set to 1.0 (for the padded size image) and that results in a variance of N in the FFT due to the scaling of the FFT,
                    // but the FFT values are divided by 1/N so the variance becomes N / (N^2) =  1/N
