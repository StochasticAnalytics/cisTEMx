...
void unblur_refine_alignment(std::vector<GpuImage>& input_stack,
    ...
    std::vector<Peak>     my_peak(max_threads);
...
#pragma omp parallel for default(shared) num_threads(max_threads) schedule(static, 1)
        for ( int image_counter = 0; image_counter < number_of_images; image_counter++ ) {
            ...
            my_peak[my_tidx] = peak_map[my_tidx].FindPeakWithParabolaFit(wanted_inner_radius_for_peak_search, outer_radius_for_peak_search);

            profile_timing_refinement_method.lap("find peak");
            // update the shifts..

            current_x_shifts[image_counter] = my_peak[my_tidx].x / float(1 + phase_multiplier);
            current_y_shifts[image_counter] = my_peak[my_tidx].y / float(1 + phase_multiplier);
