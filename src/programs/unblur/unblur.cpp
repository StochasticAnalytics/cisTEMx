...
bool UnBlurApp::DoCalculation( ) {
    ...
    int   dy_bin2                    = 0;
    int   dx_bin2                    = 0;
    float output_binning_factor_bin2 = 1.5f;
...
#pragma omp parallel for default(shared) num_threads(max_threads)
        for ( int image_counter = 0; image_counter < number_of_input_images; image_counter++ ) {
            ...
            int binned_x = unbinned_image_stack[image_counter].logical_x_dimension / pre_binning_factor;
            int binned_y = unbinned_image_stack[image_counter].logical_y_dimension / pre_binning_factor;
            if ( IsOdd(binned_x) )
                binned_x++;
            if ( IsOdd(binned_y) )
                binned_y++;
